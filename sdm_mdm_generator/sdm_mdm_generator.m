...
%% Raw data loading
input_dir = 'new_surprisal';

timestamps_struct = load([input_dir,'/timestamps.mat']);
surp_data = dlmread([input_dir,'/surprisal_data_12Jul19.txt']);
...
sem_surp_noFW = surp_data(:,3);
prob_cs = surp_data(:,4);
sem_dist = surp_data(:,5);
...
snf = sem_surp_noFW; %semantic with 0 value for function words
snf(snf>0) = zscore(-log10(snf(snf>0)));
...
[fw_des_full,fw_des_sem,fw_des_lex,fw_des_probsem, fw_des_semdist] = create_preds(hrf,tt,t,fw_data,fss);
...
[bw_des_full,bw_des_sem,bw_des_lex,bw_des_probsem,bw_des_semdist]= create_preds(hrf,tt,t,bw_data,fss);
...
prob_list_names = {'LogSemProb','InvLogSemProb','Freq','InvFreq','Dur','InvDur','Env','InvEnv','Costant'};
semdist_list_names = {'SemDist','InvSemDist','Freq','InvFreq','Dur','InvDur','Env','InvEnv','Costant'};
...
create_sdm('fw_env.sdm','fw_probsem.sdm',outdir,fw_des_probsem,prob_list_names,'fw')
create_sdm('fw_env.sdm','fw_semdist.sdm',outdir,fw_des_semdist,semdist_list_names,'fw')
...
create_sdm_noenv('fw_env.sdm','fw_probsem_noenv.sdm',outdir,fw_des_probsem,prob_list_names,'fw')
create_sdm_noenv('fw_env.sdm','fw_semdist_noenv.sdm',outdir,fw_des_semdist,semdist_list_names,'fw')
...
create_sdm('bw_env.sdm','bw_probsem.sdm',outdir,bw_des_probsem,prob_list_names,'bw')
create_sdm('bw_env.sdm','bw_semdist.sdm',outdir,bw_des_semdist,semdist_list_names,'bw')
...
create_sdm_noenv('bw_env.sdm','bw_probsem_noenv.sdm',outdir,bw_des_probsem,prob_list_names,'bw')
create_sdm_noenv('bw_env.sdm','bw_semdist_noenv.sdm',outdir,bw_des_semdist,semdist_list_names,'bw')
...
create_mdm('natexp_env.mdm','fw_probsem.sdm','bw_probsem.sdm','probsem_model.mdm',outdir)
create_mdm('natexp_env.mdm','fw_semdist.sdm','bw_semdist.sdm','24jul_semdist_model.mdm',outdir)
...
create_mdm('natexp_env.mdm','fw_probsem_noenv.sdm','bw_probsem_noenv.sdm','probsem_model_noenv.mdm',outdir)
create_mdm('natexp_env.mdm','fw_semdist_noenv.sdm','bw_semdist_noenv.sdm','24jul_semdist_model_noenv.mdm',outdir)
...
function [des_full,des_sem,des_lex,des_semprob,des_semdist]= create_preds(hrf,tt,t,data,fss)
    ...
    ref_sp = zeros(size(hrf));
    ref_sd = zeros(size(hrf));
    ...
    for i = 1 : length(timestamps)
        ...
        ref_sp(iws:iwe) = data.sp(i);
        ref_sd(iws:iwe) = data.sd(i);
    ...
    spc = conv(ref_sp,hrf,'full'); spc = spc(1:length(tt));
    sdc = conv(ref_sd,hrf,'full'); sdc = sdc(1:length(tt));
    ...
    spci = interp1(tt,spc,t);
    sdci = interp1(tt,sdc,t);
    ...
    [des_semprob,~] = gsog([spci',frci',refci',envci']);
    [des_semdist,~] = gsog([sdci',frci',refci',envci']);
