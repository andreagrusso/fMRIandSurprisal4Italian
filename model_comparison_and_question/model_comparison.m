...
for voi_idx = 1:NrOfVOIs
    ...
    sws_fixed = table(y,subs_id,sws_data(:,3),sws_data(:,4),sws_data(:,5),...
        sws_data(:,6),sws_data(:,7),sws_data(:,8),...
        'VariableNames',{'SubjData','SubsName','Freq','InvFreq','Dur','InvDur','Env','InvEnv'});
    ls_fixed = table(y,subs_id,sws_data(:,3),sws_data(:,4),sws_data(:,5),...
        sws_data(:,6),sws_data(:,7),sws_data(:,8),...
        'VariableNames',{'SubjData','SubsName','Freq','InvFreq','Dur','InvDur','Env','InvEnv'});
    %tables of the two models
    tbl_sws = table(y,subs_id,sws_data(:,1),sws_data(:,2),sws_data(:,3),sws_data(:,4),sws_data(:,5),...
        sws_data(:,6),sws_data(:,7),sws_data(:,8),...
    ...
    sws_m =  fitlme(tbl_sws, 'SubjData ~ SwS + InvSwS + Freq + InvFreq + Dur + InvDur + Env + InvEnv + (1|SubsName)');
    ls_m =  fitlme(tbl_ls, 'SubjData ~ LS + InvLS + Freq + InvFreq + Dur + InvDur + Env + InvEnv + (1|SubsName)');
